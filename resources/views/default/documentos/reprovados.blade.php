...
@section('content')
    ...
    <section class="content-header">
        <h1>
            Lista de usuários com documentos reprovados<br>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ route('home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
            <li>Documentos</li>
            <li>Associado</li>
            <li class="active">Reprovados</li>
        </ol>
    </section>
                                ...
                                <tr id="contrato-{{ $usuario->id }}">
                                    ...
                                    <td>
                                        @if(($usuario->status_cpf ==  'recusado' || $usuario->status_comprovante_endereco == 'recusado' || $usuario->status_selfie == 'recusado')
                                            && ($usuario->dadosBancarios->count() == 0 || $usuario->dadosBancarios->where('status_comprovante', 'recusado')->count() > 0)
                                            && ($usuario->responsavel_doc_reprovado > 0))
                                            Dados Pessoais, Dados Bancários e Dados do Responsável
                                        @elseif(($usuario->status_cpf ==  'recusado' || $usuario->status_comprovante_endereco == 'recusado' || $usuario->status_selfie == 'recusado')
                                            && ($usuario->dadosBancarios->count() == 0 || $usuario->dadosBancarios->where('status_comprovante', 'recusado')->count() > 0))
                                            Dados Pessoais e Dados Bancários
                                        @elseif(($usuario->status_cpf ==  'recusado' || $usuario->status_comprovante_endereco == 'recusado' || $usuario->status_selfie == 'recusado')
                                            && ($usuario->responsavel_doc_reprovado > 0))
                                            Dados Pessoais e Dados do Responsável
                                        @elseif(($usuario->dadosBancarios->count() == 0 || $usuario->dadosBancarios->where('status_comprovante', 'recusado')->count() > 0)
                                            && ($usuario->responsavel_doc_reprovado > 0))
                                            Dados Bancários e Dados do Responsável
                                        @elseif($usuario->dadosBancarios->count() == 0 || $usuario->dadosBancarios->where('status_comprovante', 'recusado')->count() > 0)
                                            Dados Bancários
                                        @elseif($usuario->responsavel_doc_reprovado > 0)
                                    ...
                                    <td>
                                        <div class="btn-group" role="group" aria-label="Botões de Ação">
                                            ...
                                            <a class="btn btn-success" href="{{route('documentos.associado.reprovados.visualizacao', $usuario->id)}}">
                                                Visualizar
