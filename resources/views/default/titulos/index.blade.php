                    ...
                    <div class="box-body">
                        <table id="tabela_index" class="table table-bordered table-striped">
                            ...
                            <tr>
                                ...
                                <th>Recebe Pontos?</th>
                                <th>Teto Fin. Mensal</th>
                                <th>Bonus Patrocinador</th>
                                <th>Ações</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($data as $dt)
                                <tr>
                                    <td>{{ $dt->id }}</td>
                                    <td>{{ $dt->name }}</td>
                                    <td>{{ $dt->tituloSuperior ? $dt->tituloSuperior->name : 'Nenhum' }}</td>
                                    <td>{{ $dt->recebe_pontuacao ? 'Sim' : 'Não' }}</td>
                                    <td>{{ mascaraMoeda($sistema->moeda, $dt->teto_mensal_financeiro, 2, true) }}</td>
                                    <td>{{ $dt->bonus_indicador }}</td>
                                    <td>
                                        <form method="post" id="formDel" action="{{ route('titulo.destroy', $dt->id) }}">
                                            {!! csrf_field() !!}
                                            <div class="btn-group" role="group" aria-label="Botões de Ação">
                                                <a title="Editar" class="btn btn-default btn-sm" href="{{ route('titulo.edit', $dt->id) }}">
