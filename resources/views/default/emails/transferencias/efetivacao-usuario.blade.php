...
<div class="preheader" style="display: none; max-width: 0; max-height: 0; overflow: hidden; font-size: 1px; line-height: 1px; color: #fff; opacity: 0;">
  Esta é uma notificação de efetivação de transferência realizado em sua conta {{ env('COMPANY_NAME_SHORT', 'empresa') }}. Você está recebendo este e-mail para garantir que a operação foi feito por você.
</div>
<!-- end preheader -->
@endsection
            ...
            <tr>
                <td align="center" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
                    @if($transferencia->destinatario_user_id)
                        <p style="margin: 0;"><b>Transferência entre contas {{ ucfirst(env('COMPANY_NAME_SHORT', 'empresa')) }} realizada com sucesso:</b></p>
                        <p>
                            enviada para <br>
                            <h1>{{ $transferencia->destinatario->name }}</h1>
                        </p>
                        <p>
                            <b>no valor de</b>
                            <h1>{{ mascaraMoeda('R$', $transferencia->valor, 2, true) }}</h1>
                            <b>CPF/CNPJ</b>: {{ $transferencia->destinatario->cpf }} <br>
                            <b>Agência</b>: 0001 <br>
                            <b>Conta</b>: {{ $transferencia->destinatario->conta }}
                        </p>
                    @else
                        <p style="margin: 0;"><b>Transferência para conta bancária efetivada com sucesso</b></p> <br>
                    ...
                    <p>Se não foi você que realizou a transferência, altere sua senha imediatamente e fale com nossa equipe pelo link: <a href="{{ $empresa->site }}" target="
                    ">{{ $empresa->site }}</a></p>
