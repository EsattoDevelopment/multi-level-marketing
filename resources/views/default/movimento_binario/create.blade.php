        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <!-- form start -->
                    <form role="form" action="{{ route('movimento.binario.store') }}" method="post">
                        {!! csrf_field() !!}
                        <div class="box-body">
                            <div class="form-group col-xs-12">
                                ...
                                <select class="form-control select2"  name="operacao_id" data-placeholder="Selecione uma operação" style="width: 100%;">
                                    ...
                                    @foreach($operacoes as $operacao)
                                        <option @if(old('operacao_id') == $operacao->id) selected @endif value="{{ $operacao->id }}">{{ $operacao->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-xs-12">
                                <label for="exampleInputEmail1">Valor</label><br>
                                <small><i>Sem pontos</i></small>
                            ...
                            <div class="form-group col-xs-12">
                                <label>Lado</label>
                                <select class="form-control select2" required  name="lado" data-placeholder="Selecione um lado" style="width: 100%;">
                            ...
                            <div class="form-group col-xs-12">
                                ...
                                <select class="form-control select2"  name="user_id" data-placeholder="Selecione um usuário" style="width: 100%;">
                                    ...
                                    @foreach($usuarios as $usuario)
                                        <option value="{{ $usuario->id }}">#{{ $usuario->id }} - {{ $usuario->name }} - {{ $usuario->username }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div><!-- /.box-body -->
