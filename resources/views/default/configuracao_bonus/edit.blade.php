            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">Edição de Configuração de Bônus</h3>
                    </div><!-- /.box-header -->
                    <!-- form start -->
                    <form role="form" action="{{ route('configuracao-bonus.update', $dados->id) }}" method="post">
                        {!! csrf_field() !!}
                        <div class="box-body">
                            <div class="form-group col-xs-12">
                                ...
                                <label style="padding-right: 25px">
                                    <input type="radio" value="1" name="status" class="flat-red" {{ old('status', $dados->status)  == 1 ? 'checked' : '' }}>
                                    Ativo
                                </label>
                                <label>
                                    <input type="radio" value="0" name="status" class="flat-red" {{ old('status', $dados->status)  == 0 ? 'checked' : '' }}>
                                    Inativo
                                </label>
                            </div>
                            <div class="form-group col-xs-12">
                            ...
                            <div class="form-group col-xs-12">
                                <label for="exampleInputEmail1">Títulos Ligados a essa configuração</label><br>
                                <table id="tabela_index" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach($dados->titulosBonusAdesao as $adesao)
                                            <tr>
                                                <td>{{$adesao->name}}</td>
                                                <td>Bônus de adesão</td>
                                            </tr>
                                        @endforeach
                                        @foreach($dados->titulosBonusRentabilidade as $rent)
                                            <tr>
                                                <td>{{$rent->name}}</td>
                                                <td>Bônus de rentabilidade</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                                ...
                                <table id="tabela_index" class="table table-bordered table-striped">
                                    ...
                                    @for($i=1; $i<=$sistema->profundidade_pagamento_matriz; $i++)
                                        <tr>
                                            <td>{{$i}}</td>
                                            <td>
                                                <input style="text-align: center;" type="text" id="valor_fixo_{{$i}}" name="itens[{{$i}}][valor_fixo]" value="{{ old('itens.'.$i.'.valor_fixo', $dados->itens[$i]['valor_fixo'] ?? '0.00') }}" class="form-control somar_fixo" placeholder="Valor Fixo">
                                            </td>
                                            <td>
                                                <input style="text-align: center;" type="text" id="percentual_{{$i}}" name="itens[{{$i}}][percentual]" value="{{ old('itens.'.$i.'.percentual', $dados->itens[$i]['percentual'] ?? '0') }}" class="form-control somar_percentual" placeholder="Percentual %">
                                            </td>
                                            <td>
