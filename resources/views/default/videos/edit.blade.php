...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"><i class="glyphicon glyphicon-edit"></i> Edição</li>
        </ol>
    </section>
    <section class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <!-- form start -->
                <form role="form" action="{{ route('videos.update', $video->id) }}" method="post" enctype="multipart/form-data">
                {!! csrf_field() !!}
                        ...
                        <div class="box-body">
                            <div class="form-group col-xs-6 col-lg-6">
                                <div class="form-group">
                                    <label>Nome</label>
                                    <input type="text" name="nome" tabIndex="1" class="form-control" placeholder="Digite aqui um nome para o vídeo ..." value="{{old('nome',$video->nome)}}" required>
                                </div>
                                <div class="form-group">
                                    <label>Vídeo</label>
                                    <div class="form-group input-group">
                                        ...
                                        <input type="hidden" name="tipo" id="tipo" value="{{old('tipo', $video->tipo)}}">

                                        <input type="text" tabIndex="2" class="form-control" placeholder="Cole aqui a url do vídeo ..." name="codigo" id="codigo" onfocusout="verificarTipo()" value="{{$url . old('codigo', $video->codigo)}}" required>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="categoria" tabIndex="3" class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="9" tabindex="-1" aria-hidden="true">
                                        ...
                                        @foreach(config('constants.videos_categorias') as $categoriaId => $categoria)
                                            <option value="{{$categoriaId}}" {{ old('categoria', $video->categoria) == $categoriaId ? 'selected="selected"' : '' }} data-select2-id="{{$categoriaId}}">{{$categoria}}</option>
                                        @endforeach
                                        {{--
                                        @if($video->categoria == 1)
                                            ...
                                            @endforeach
                                        @else
                                            <option value="1" data-select2-id="1">Rendimento diario</option>
                                            @foreach($categorias as $categoria => $categoriaId)
                                                <option value="{{$categoriaId}}" @if($video->categoria == $categoriaId) selected="selected" @endif data-select2-id="{{$categoriaId}}">{{$categoria}}</option>
                                            @endforeach
                                        @endif--}}
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea id="descricao" name="descricao" tabIndex="4" class="form-control" rows="3" placeholder="Digite aqui uma descrição para o vídeo ...">{{old('descricao', $video->descricao)}}</textarea>
                                </div>
                                <div class="form-group col-xs-6 col-lg-6">
                                    <label for="status">Ativo</label> <br>
                                    <div class="btn-group" data-toggle="buttons">
                                        <label class="iradio_flat-red">
                                            <input type="radio" value="1" name="status" @if('status' ? old('status', $video->status) == 1 : false) checked @endif>
                                            Sim
                                        </label>
                                        <label class="iradio_flat-red">
                                            <input type="radio" value="0" name="status" @if('status' ? old('status', $video->status) == 0 : false) checked @endif>
                                            Não
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class="form-group col-xs-12 col-lg-12">
                                @foreach($titulos as $titulo)
                                    <div class="form-group col-xs-12 col-lg-3 text-center">
                                        ...
                                        <div class="text-center">
                                            <input type="checkbox" @if('titulosPermissoes['.$titulo->id.'][exibir]' ? old("titulosPermissoes[{$titulo->id}][exibir]", in_array($titulo->id, $permissoes)) : in_array($titulo->id, $permissoes)) checked @endif value="1" name="titulosPermissoes[{{$titulo->id}}][exibir]" tabIndex="5" class="icheckbox_flat-red" >
                                        </div>
                                    </div>
                                @endforeach
                            </div>
        ...
        function verificarTipo() {
            ...
            if(tipoVideo != null) {
                ...
                if ((/youtube|youtu/gi).test(codigo.value)) {
                    ...
                    tipo.value = 2;
                    htmlVisualizador = '<iframe src="https://player.vimeo.com/video/' + getVideoCode() + '" width="100%" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>'
                    ...
                    tipo.value = 0;
                    htmlVisualizador = '<video controls width="100%" height="auto"> <source src="' + codigo.value + '" type="video/mp4"> </video>';
                    window.document.getElementById('player').innerHTML = htmlVisualizador;
                }
            }
        };
