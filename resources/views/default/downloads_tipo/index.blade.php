...
@section('content')
    ...
    <section class="content-header">
        <h1>
            Lista de Tipo de Downloads
        </h1>
        <ol class="breadcrumb">
            <li><a href="{{ route('home') }}"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Tipo de Downloads</li>
        </ol>
    </section>
                                    ...
                                    <table id="tabela_portugues" class="table table-bordered table-striped">
                                        ...
                                        @foreach($dados as $dd)
                                            <tr>
                                                ...
                                                <td>{{ $dd->titulo }}</td>
                                                <td>
                                                    <form method="post" id="formDel_{{ $dd->id }}" action="{{ route('download-tipo.destroy', $dd->id) }}">
                                                        {!! csrf_field() !!}
                                                        <div class="btn-group" role="group" aria-label="Botões de Ação">
                                                            <a title="Editar" class="btn btn-default btn-sm" href="{{ route('download-tipo.edit', $dd->id) }}">
                                                                <span class="glyphicon glyphicon-edit text-success" aria-hidden="true"></span>
                                                            </a>
                                                            <a title="Desativar" class="btn btn-default btn-sm" href="{{ route('download-tipo.delete', $dd->id) }}">
                        ...
                        <div class="tab-pane" id="desativados">
                            <div class="box">
                                <div class="box-header text-danger">
                                    Lista de Tipo de Downloads desativadas
                                </div>
                                <!-- /.box-header -->
                                <div class="box-body">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <form method="post" id="formDel_{{ $dd->id }}" action="{{ route('download-tipo.destroy', $dd->id) }}">
                                                        {!! csrf_field() !!}
                                                        <div class="btn-group" role="group" aria-label="Botões de Ação">
                                                            <a title="Ativar" class="btn btn-default btn-sm" href="{{ route('download-tipo.recovery', $dd->id) }}">
...
{{--    <script src="/js/backend/list.js" type="text/javascript"></script>--}}
    <script type="text/javascript">
        $(function() {
            ...
            $('<div class="btn-group"><a href="{{ route('download-tipo.create') }}" class="btn btn-primary">Novo</a></div>').appendTo('div.box-btn');
        })
    </script>
@endsection