<?php

/*
 * Esse arquivo faz parte de <MasterMundi/Master MDR>
 * (c) Nome Autor zehluiz17[at]gmail.com
 *
 */

namespace App\Saude\Listeners\Contrato;

use App\Models\Movimentos;
use Illuminate\Support\Facades\Auth;
use App\Saude\Events\CancelamentoContrato;

class EstornoBonus
{
    /**
     * Create the event listener.
     *
     * @return void
     */
    public function __construct()
    {
        //
    }

    /**
     * Handle the event.
     *
     * @param  CancelamentoContrato $event
     * @return void
     */
    public function handle(CancelamentoContrato $event)
    {
        \Log::info('####Inicio estorno Bonus');

        if (7 == $event->getContato()->status) {
            $item = $event->getContato()->item;

            // só tem bonus se o pedido for do tipo adesão
            if (1 == $item->tipo_pedido_id) {
                $pedido = $event->getContato()->pedido;

                $movimento = $pedido->movimento;

                if (! $movimento) {
                    $movimento = $pedido->movimentoRef;
                }

                $user = $movimento->usuario;

                $lastMovimentos = $user->ultimoMovimento();

                $dadosMovimento = [
                    'valor_manipulado'    => -$movimento->valor_manipulado,
                    'saldo_anterior'      => $lastMovimentos->saldo,
                    'saldo'               => $lastMovimentos->saldo - $movimento->valor_manipulado,
                    'pedido_id'           => $pedido->id,
                    'descricao'           => 'Estorno de cancelamento de contrato (Bônus) #'.$event->getContato()->id,
                    'responsavel_user_id' => Auth::user()->id,
                    'user_id'             => $movimento->user_id,
                    'operacao_id'         => 22,
                ];

                Movimentos::create($dadosMovimento);
            } else {
                \Log::info('Não há nada para estornar');
            }
        } else {
            \Log::info('Não foi cancelado dentro do prazo');
        }
        \Log::info('####Fim estorno Bonus');
    }
}
